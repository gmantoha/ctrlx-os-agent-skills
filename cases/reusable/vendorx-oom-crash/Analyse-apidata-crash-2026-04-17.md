# Analyse: ctrlx-vendorx-vendorxdc apidata-Crash vom 17.04.2026

**Plattform:** Bosch Rexroth ctrlX M4 (ARM64), Kernel `6.1.80-rt26-ctrlx-43`  
**Betroffener Snap:** `ctrlx-vendorx-vendorxdc` (Revision x12)  
**Betroffener Service:** `snap.ctrlx-vendorx-vendorxdc.apidata.service`  
...
Der `apidata`-Service des VendorX-Snaps ist **nicht** durch einen OOM-Kill abgestuerzt, sondern durch einen **InfluxDB-Timeout** waehrend einer blockierenden TSM-Cache-Snapshot-Operation. InfluxDB war fuer ca. 31 Sekunden nicht ansprechbar, was zu kaskadenartigen Fehlern im gesamten System fuehrte.
...
Exception in thread Thread-1 (loop):
  File ".../vendorxapi/poller/poller.py", line 215, in loop
    results = self.influxdb.query_data("vendorxapi", self.start_period, self.stop_period)
...
```
  File ".../vendorxapi/poller/poller.py", line 361, in main
    poller_thread = create_poller()
  File ".../vendorxapi/poller/poller.py", line 334, in create_poller
    poller = DataPoller()
  File ".../vendorxapi/poller/poller.py", line 138, in __init__
...
snap.ctrlx-vendorx-vendorxdc.apidata.service: Main process exited, code=exited, status=1/FAILURE
```
...
## Handlungsempfehlungen fuer VendorX
...
### 4. Vorbestehendes Problem: rda-proxy-service (Prioritaet niedrig, nicht VendorX-spezifisch)

Die Fehlermeldung `proxy.sock: connect: connection refused` tritt alle 60 Sekunden auf und betrifft den `rexroth-deviceadmin`-Snap. Dies ist ein separates Problem auf der ctrlX-Plattform, nicht im VendorX-Snap.
...
|---|---|---|
| `lib/python3.10/site-packages/vendorxapi/poller/poller.py` | 138, 215, 334, 361 | Fehlende Fehlerbehandlung bei InfluxDB-Timeouts |
